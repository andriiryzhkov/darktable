...
struct sam_hparams
{
  ...
  {
    return n_enc_state / n_enc_head;
  }
  int32_t n_img_size() const
  {
    return 1024;
  }
  int32_t n_window_size() const
  {
    return 14;
  }
...
bool sam_ggml_model_load(const sam_params &params, sam_ggml_model &model)
{
  ...
  const size_t ctx_size = [&]()
  {
    ...
    // image encoder
    {
      result_size += n_enc_state * n_img_embd * n_img_embd * ggml_type_size(GGML_TYPE_F32);

      result_size += n_enc_state * 3 * n_patch_size * n_patch_size * ggml_type_size(GGML_TYPE_F16);
      result_size += n_enc_state * ggml_type_size(GGML_TYPE_F32);

      result_size += n_enc_state * n_enc_out_chans * 1 * 1 * ggml_type_size(GGML_TYPE_F16);
      result_size += n_enc_out_chans * n_enc_out_chans * 3 * 3 * ggml_type_size(GGML_TYPE_F16);

      result_size += n_enc_out_chans * ggml_type_size(GGML_TYPE_F32);
      result_size += n_enc_out_chans * ggml_type_size(GGML_TYPE_F32);
    ...
    {
      result_size += n_enc_layer * n_enc_state * ggml_type_size(GGML_TYPE_F32);
      result_size += n_enc_layer * n_enc_state * ggml_type_size(GGML_TYPE_F32);

      result_size += n_enc_layer_global * n_enc_head_dim * (2 * n_img_embd - 1) * ggml_type_size(GGML_TYPE_F16);
      result_size += n_enc_layer_global * n_enc_head_dim * (2 * n_img_embd - 1) * ggml_type_size(GGML_TYPE_F16);

      result_size += n_enc_layer_local * n_enc_head_dim * (2 * n_window_size - 1) * ggml_type_size(GGML_TYPE_F16);
      result_size += n_enc_layer_local * n_enc_head_dim * (2 * n_window_size - 1) * ggml_type_size(GGML_TYPE_F16);

      result_size += n_enc_layer * 3 * n_enc_state * n_enc_state * ggml_type_size(GGML_TYPE_F16);
      result_size += n_enc_layer * 3 * n_enc_state * ggml_type_size(GGML_TYPE_F32);

      result_size += n_enc_layer * n_enc_state * n_enc_state * ggml_type_size(GGML_TYPE_F16);
      result_size += n_enc_layer * n_enc_state * ggml_type_size(GGML_TYPE_F32);

      result_size += n_enc_layer * n_enc_state * ggml_type_size(GGML_TYPE_F32);
      result_size += n_enc_layer * n_enc_state * ggml_type_size(GGML_TYPE_F32);

      result_size += n_enc_layer * 4 * n_enc_state * n_enc_state * ggml_type_size(GGML_TYPE_F16);
      result_size += n_enc_layer * 4 * n_enc_state * ggml_type_size(GGML_TYPE_F32);

      result_size += n_enc_layer * 4 * n_enc_state * n_enc_state * ggml_type_size(GGML_TYPE_F16);
      result_size += n_enc_layer * 4 * n_enc_state * ggml_type_size(GGML_TYPE_F32);
    }

    result_size += (8 + 14 * n_enc_layer) * ggml_tensor_overhead();

    // prompt encoder
    {
      result_size += n_enc_out_chans * ggml_type_size(GGML_TYPE_F16); // 2*(n_enc_out_chans/2)

      result_size += n_enc_out_chans * ggml_type_size(GGML_TYPE_F32);
      result_size += n_pt_embd * n_enc_out_chans * ggml_type_size(GGML_TYPE_F32);
    }

    result_size += (2 + n_pt_embd) * ggml_tensor_overhead();
    ...
    // mask decoder
    {
      // transformer
      {
        ...
        // self_attn
        result_size += tfm_layers_count * qkv_count * n_enc_state * n_enc_state * ggml_type_size(GGML_TYPE_F16);
        result_size += tfm_layers_count * qkv_count * n_enc_state * ggml_type_size(GGML_TYPE_F32);
        result_size += tfm_layers_count * n_enc_state * ggml_type_size(GGML_TYPE_F32);

        // all norms
        result_size += tfm_layers_count * norm_count * n_enc_state * ggml_type_size(GGML_TYPE_F32);
        result_size += tfm_layers_count * norm_count * n_enc_state * ggml_type_size(GGML_TYPE_F32);

        // cross_attn_token_to_img
        result_size += tfm_layers_count * qkv_count * n_enc_state * (n_enc_state / 2) * ggml_type_size(GGML_TYPE_F16);
        result_size += tfm_layers_count * qkv_count * (n_enc_state / 2) * ggml_type_size(GGML_TYPE_F32);
        result_size += tfm_layers_count * n_enc_state * ggml_type_size(GGML_TYPE_F32);

        // mlp
        result_size += tfm_layers_count * 8 * n_enc_out_chans * n_enc_out_chans * ggml_type_size(GGML_TYPE_F16);
        result_size += tfm_layers_count * 8 * n_enc_out_chans * ggml_type_size(GGML_TYPE_F32);
        result_size += tfm_layers_count * n_enc_out_chans * 8 * n_enc_out_chans * ggml_type_size(GGML_TYPE_F16);
        result_size += tfm_layers_count * n_enc_out_chans * ggml_type_size(GGML_TYPE_F32);
        ...
        result_size += qkv_count * n_enc_state * (n_enc_state / 2) * ggml_type_size(GGML_TYPE_F16);
        result_size += qkv_count * (n_enc_state / 2) * ggml_type_size(GGML_TYPE_F32);
        result_size += n_enc_state * ggml_type_size(GGML_TYPE_F32);

        // transformer_norm_final
        result_size += norm_count * n_enc_state * ggml_type_size(GGML_TYPE_F32);
        result_size += norm_count * n_enc_state * ggml_type_size(GGML_TYPE_F32);

        // output_upscaling
        result_size += n_enc_out_chans * n_img_embd * 2 * 2 * ggml_type_size(GGML_TYPE_F16);
        result_size += 3 * n_img_embd * ggml_type_size(GGML_TYPE_F32);
        result_size += n_enc_out_chans * n_img_embd * (n_img_embd / 2) * 2 * 2 * ggml_type_size(GGML_TYPE_F16);
        result_size += (n_img_embd / 2) * ggml_type_size(GGML_TYPE_F32);

        // output_hypernetworks_mlps
        result_size += n_hypernet_mpls_count * 2 * n_enc_out_chans * n_enc_out_chans * ggml_type_size(GGML_TYPE_F16);
        result_size += n_hypernet_mpls_count * 2 * n_enc_out_chans * ggml_type_size(GGML_TYPE_F32);
        result_size += n_hypernet_mpls_count * n_enc_out_chans * (n_img_embd / 2) * ggml_type_size(GGML_TYPE_F16);
        result_size += n_hypernet_mpls_count * (n_img_embd / 2) * ggml_type_size(GGML_TYPE_F32);

        // iou_prediction_head
        result_size += 2 * n_enc_out_chans * n_enc_out_chans * ggml_type_size(GGML_TYPE_F16);
        result_size += 2 * n_enc_out_chans * ggml_type_size(GGML_TYPE_F32);
        result_size += n_pt_embd * n_enc_out_chans * ggml_type_size(GGML_TYPE_F16);
        result_size += n_pt_embd * ggml_type_size(GGML_TYPE_F32);
    ...
    // mask decoder
    {
      ...
      for(int i = 0; i < tfm_layers_count; ++i)
      {
        ...
        l.cross_attn_token_to_img.q_w
            = ggml_new_tensor_2d(ctx, GGML_TYPE_F16, n_enc_out_chans, n_enc_out_chans / 2);
        ...
        l.cross_attn_token_to_img.out_w
            = ggml_new_tensor_2d(ctx, GGML_TYPE_F16, n_enc_out_chans / 2, n_enc_out_chans);
      ...
      model.tensors["mask_decoder.transformer.final_attn_token_to_image.q_proj.weight"]
          = dec.transformer_final_attn_token_to_img.q_w;
      model.tensors["mask_decoder.transformer.final_attn_token_to_image.q_proj.bias"]
          = dec.transformer_final_attn_token_to_img.q_b;
      model.tensors["mask_decoder.transformer.final_attn_token_to_image.k_proj.weight"]
          = dec.transformer_final_attn_token_to_img.k_w;
      model.tensors["mask_decoder.transformer.final_attn_token_to_image.k_proj.bias"]
          = dec.transformer_final_attn_token_to_img.k_b;
      model.tensors["mask_decoder.transformer.final_attn_token_to_image.v_proj.weight"]
          = dec.transformer_final_attn_token_to_img.v_w;
      model.tensors["mask_decoder.transformer.final_attn_token_to_image.v_proj.bias"]
          = dec.transformer_final_attn_token_to_img.v_b;
      model.tensors["mask_decoder.transformer.final_attn_token_to_image.out_proj.weight"]
          = dec.transformer_final_attn_token_to_img.out_w;
      model.tensors["mask_decoder.transformer.final_attn_token_to_image.out_proj.bias"]
...
struct ggml_tensor *sam_fill_dense_pe(const sam_ggml_model &model, struct ggml_context *ctx0,
                                      struct ggml_cgraph *gf, sam_ggml_state &state)
{
  ...
  // https://github.com/facebookresearch/segment-anything/blob/main/segment_anything/modeling/prompt_encoder.py#L192
  {
    ...
    ggml_build_forward_expand(gf, ggml_cpy(ctx0, t_sin,
                                           ggml_view_3d(ctx0, cur, t_sin->ne[0], t_sin->ne[1], t_sin->ne[2],
                                                        cur->nb[1], cur->nb[2], 0)));
    ggml_build_forward_expand(gf, ggml_cpy(ctx0, t_cos,
                                           ggml_view_3d(ctx0, cur, t_sin->ne[0], t_sin->ne[1], t_sin->ne[2],
                                                        cur->nb[1], cur->nb[2], t_sin->nb[1])));
  }
...
struct ggml_tensor *sam_layer_norm_2d(struct ggml_context *ctx0, struct ggml_tensor *layer, int n_channels,
                                      struct ggml_tensor *w, struct ggml_tensor *b, float eps)
{
  // LayerNorm2d
  // normalize along channel dimmension
  // TODO: better implementation
  layer = ggml_permute(ctx0, ggml_norm(ctx0, ggml_cont(ctx0, ggml_permute(ctx0, layer, 1, 2, 0, 3)), eps), 2, 0, 1,
                       3);
...
struct ggml_cgraph *sam_encode_image(const sam_ggml_model &model, sam_ggml_state &state, const sam_image_f32 &img)
{
  ...
  for(int il = 0; il < n_enc_layer; ++il)
  {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor *rel_w = ggml_cont(
          ctx0, ggml_permute(ctx0, ggml_mul_mat(ctx0, rw, ggml_cont(ctx0, ggml_permute(ctx0, q_r, 0, 2, 1, 3))), 0,
                             2, 1, 3));
      ...
      cur = ggml_reshape_4d(
          ctx0,
          ggml_cont(ctx0, ggml_permute(ctx0, ggml_reshape_4d(ctx0, KQV, n_enc_head_dim, W * H, n_enc_head, B), 0,
                                       2, 1, 3)),
          n_enc_state, W, H, B);
  ...
  {
    struct ggml_tensor *inpt = ggml_graph_get_tensor(gf, "inp");
    float *data = (float *)ggml_get_data(inpt);
...
//
prompt_encoder_result sam_encode_prompt(const sam_ggml_model &model, struct ggml_context *ctx0,
                                        struct ggml_cgraph *gf, sam_ggml_state &state,
                                        const std::vector<sam_point> &points)
{
  ...
  // https://github.com/facebookresearch/segment-anything/blob/main/segment_anything/modeling/prompt_encoder.py#L192
  {
    ...
    ggml_build_forward_expand(
        gf, ggml_cpy(ctx0, t_sin, ggml_view_2d(ctx0, cur, t_sin->ne[0], t_sin->ne[1], cur->nb[1], 0)));
    ggml_build_forward_expand(
        gf, ggml_cpy(ctx0, t_cos, ggml_view_2d(ctx0, cur, t_sin->ne[0], t_sin->ne[1], cur->nb[1], t_sin->nb[1])));
    ...
    ggml_build_forward_expand(
        gf, ggml_cpy(ctx0, enc.not_a_pt_embd_w,
                     ggml_view_2d(ctx0, cur, cur->ne[0], 1, cur->nb[1], points.size() * cur->nb[1])));
  }

  // add point embeddings based on label (positive: pt_embd[1], negative: pt_embd[0])
  // ref:
  ...
  for(size_t i = 0; i < points.size(); i++)
  {
    ...
    // Use pt_embd[1] for positive points (label=1) and pt_embd[0] for negative points (label=0)
    struct ggml_tensor *embd = points[i].label > 0 ? enc.pt_embd[1] : enc.pt_embd[0];
  ...
  struct ggml_tensor *embd_prompt_dense
      = ggml_repeat(ctx0,
                    ggml_cont(ctx0, ggml_view_3d(ctx0, enc.no_mask_embd_w, 1, 1, enc.no_mask_embd_w->ne[0],
                                                 enc.no_mask_embd_w->nb[0], enc.no_mask_embd_w->nb[0], 0)),
                    ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, hparams.n_img_embd(), hparams.n_img_embd(),
...
struct ggml_tensor *sam_decode_mask_transformer_attn(const sam_layer_dec_transformer_attn &attn,
                                                     struct ggml_tensor *queries, struct ggml_tensor *keys,
                                                     struct ggml_tensor *values, struct ggml_context *ctx0,
                                                     const sam_ggml_model &model)
{
  ...
  KQV_merged = ggml_reshape_3d(ctx0, KQV_merged, KQV_merged->ne[0] * KQV_merged->ne[1], KQV_merged->ne[2],
                               KQV_merged->ne[3]);
...
struct ggml_tensor *sam_decode_mask_mlp_relu_3(struct ggml_tensor *in, struct ggml_tensor *w_0,
...
bool sam_decode_mask(const sam_ggml_model &model, const prompt_encoder_result &prompt, struct ggml_tensor *pe_img,
                     struct ggml_context *ctx0, struct ggml_cgraph *gf, sam_ggml_state &state)
{
  ...
  struct ggml_tensor *tokens = {};
  {
    ...
    tokens = ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, dec.iou_token_w->ne[0],
                                dec.iou_token_w->ne[1] + dec.mask_tokens_w->ne[1] + sparse->ne[1], sparse->ne[2]);

    const size_t offsets[3]
        = { 0, dec.iou_token_w->ne[1] * tokens->nb[1],
            dec.iou_token_w->ne[1] * tokens->nb[1] + dec.mask_tokens_w->ne[1] * tokens->nb[1] };
    ggml_build_forward_expand(gf, ggml_cpy(ctx0, dec.iou_token_w,
                                           ggml_view_2d(ctx0, tokens, tokens->ne[0], dec.iou_token_w->ne[1],
                                                        tokens->nb[1], offsets[0])));
    ggml_build_forward_expand(gf, ggml_cpy(ctx0, dec.mask_tokens_w,
                                           ggml_view_2d(ctx0, tokens, tokens->ne[0], dec.mask_tokens_w->ne[1],
                                                        tokens->nb[1], offsets[1])));
    ggml_build_forward_expand(
        gf, ggml_cpy(ctx0, sparse,
                     ggml_view_2d(ctx0, tokens, tokens->ne[0], sparse->ne[1], tokens->nb[1], offsets[2])));
  ...
  int srcNE[4] = { 0, 0, 0, 0 };
  {
    ...
    src = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, state.embd_img->ne[0], state.embd_img->ne[1],
                             state.embd_img->ne[2], tokens->ne[2]);

    src = ggml_add(ctx0, ggml_repeat(ctx0, state.embd_img, src), prompt.embd_prompt_dense);
    ...
    src = ggml_cont(ctx0, ggml_permute(ctx0,
                                       ggml_view_3d(ctx0, src, src->ne[0] * src->ne[1], src->ne[2], src->ne[3],
                                                    src->nb[2], src->nb[3], 0),
                                       1, 0, 2, 3));
    ...
    pos_src
        = ggml_cont(ctx0, ggml_permute(ctx0,
                                       ggml_view_3d(ctx0, pos_src, pos_src->ne[0] * pos_src->ne[1], pos_src->ne[2],
                                                    pos_src->ne[3], pos_src->nb[2], pos_src->nb[3], 0),
                                       1, 0, 2, 3));
  }
  ...
  struct ggml_tensor *keys = src;
  {
    ...
    for(int i = 0; i < int(model.dec.transformer_layers.size()); ++i)
    {
      ...
      else
      {
        ...
        struct ggml_tensor *self_attn
            = sam_decode_mask_transformer_attn(tfm_layer.self_attn, q_0, q_0, queries, ctx0, model);
        queries = ggml_add(ctx0, queries, self_attn);
      }

      queries = ggml_norm(ctx0, queries, hparams.eps_decoder_transformer);
      queries = ggml_add_inplace(ctx0, ggml_mul(ctx0, queries, tfm_layer.norm1_w), tfm_layer.norm1_b);
      ...
      struct ggml_tensor *cross_attn_token_to_img
          = sam_decode_mask_transformer_attn(tfm_layer.cross_attn_token_to_img, q_1, k_1, keys, ctx0, model);
      ...
      queries = ggml_add_inplace(ctx0, ggml_mul(ctx0, queries, tfm_layer.norm2_w), tfm_layer.norm2_b);
      ...
      struct ggml_tensor *mlp_out = ggml_mul_mat(ctx0, tfm_layer.mlp_lin1_w, queries);
      ...
      queries = ggml_norm_inplace(ctx0, queries, hparams.eps_decoder_transformer);
      queries = ggml_add_inplace(ctx0, ggml_mul(ctx0, queries, tfm_layer.norm3_w), tfm_layer.norm3_b);
      ...
      struct ggml_tensor *cross_attn_img_to_token
          = sam_decode_mask_transformer_attn(tfm_layer.cross_attn_img_to_token, k_2, q_2, queries, ctx0, model);
      keys = ggml_add_inplace(ctx0, keys, cross_attn_img_to_token);
      keys = ggml_norm_inplace(ctx0, keys, hparams.eps_decoder_transformer);
      keys = ggml_add_inplace(ctx0, ggml_mul(ctx0, keys, tfm_layer.norm4_w), tfm_layer.norm4_b);
    }
    ...
    struct ggml_tensor *final_attn_token_to_img
        = sam_decode_mask_transformer_attn(dec.transformer_final_attn_token_to_img, q, k, keys, ctx0, model);

    queries = ggml_add_inplace(ctx0, queries, final_attn_token_to_img);
    queries = ggml_norm_inplace(ctx0, queries, hparams.eps_decoder_transformer);
    queries = ggml_add_inplace(ctx0, ggml_mul(ctx0, queries, dec.transformer_norm_final_w),
  ...
  struct ggml_tensor *mask_tokens_out
      = ggml_view_3d(ctx0, queries, queries->ne[0], num_mask_tokens, queries->ne[2], queries->nb[1],
                     num_mask_tokens * queries->nb[1], queries->nb[1]);
  ...
  keys = ggml_view_4d(ctx0, keys, srcNE[0], srcNE[1], srcNE[2], srcNE[3], srcNE[0] * keys->nb[0], keys->nb[1],
                      keys->nb[2], 0);
  ...
  struct ggml_tensor *upscaled_embedding = {};
  {
    ...
    keys = ggml_add_inplace(
        ctx0, keys,
        ggml_repeat(ctx0, ggml_reshape_3d(ctx0, dec.output_upscaling_0_b, 1, 1, dec.output_upscaling_0_b->ne[0]),
                    keys));
    ...
    keys = ggml_add_inplace(
        ctx0,
        ggml_repeat(ctx0, ggml_reshape_3d(ctx0, dec.output_upscaling_3_b, 1, 1, dec.output_upscaling_3_b->ne[0]),
                    keys),
        keys);
    // GELU activation
    keys = ggml_gelu_inplace(ctx0, keys);
  ...
  struct ggml_tensor *hyper_in
      = ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, n_img_embd / 2, num_mask_tokens, mask_tokens_out->ne[2]);

  for(int i = 0; i < num_mask_tokens; ++i)
  {
    const auto &mlp = dec.output_hypernet_mlps[i];
    struct ggml_tensor *in = ggml_view_2d(ctx0, mask_tokens_out, mask_tokens_out->ne[0], mask_tokens_out->ne[2],
                                          mask_tokens_out->nb[1], i * mask_tokens_out->nb[1]);
    struct ggml_tensor *out
        = sam_decode_mask_mlp_relu_3(in, mlp.w_0, mlp.b_0, mlp.w_1, mlp.b_1, mlp.w_2, mlp.b_2, ctx0);
    ggml_build_forward_expand(gf, ggml_cpy(ctx0, out,
  ...
  iou_pred = sam_decode_mask_mlp_relu_3(iou_pred, dec.iou_prediction_head_0_w, dec.iou_prediction_head_0_b,
  ...
  iou_pred = ggml_cpy(state.ctx_masks, ggml_view_1d(ctx0, iou_pred, iou_pred->ne[0] - 1, iou_pred->nb[0]),
                      state.iou_predictions);
  masks = ggml_view_4d(ctx0, masks, masks->ne[0], masks->ne[1], masks->ne[2] - 1, masks->ne[3], masks->nb[1],
...
std::vector<sam_image_u8> sam_postprocess_masks(const sam_hparams &hparams, int nx, int ny,
                                                const sam_ggml_state &state, int mask_on_val, int mask_off_val)
{
...
struct ggml_cgraph *sam_build_fast_graph(const sam_ggml_model &model, sam_ggml_state &state, int nx, int ny,
...
bool sam_compute_embd_img(sam_image_u8 &img, int n_threads, sam_state &state)
{
  ...
  st.embd_img = ggml_new_tensor_3d(st.ctx_img, GGML_TYPE_F32, model.hparams.n_img_embd(),
...
std::vector<sam_image_u8> sam_compute_masks(sam_image_u8 &img, int n_threads, std::vector<sam_point> points,
                                            ...
                                            int mask_off_val)
{
  ...
  st.low_res_masks = ggml_new_tensor_3d(st.ctx_masks, GGML_TYPE_F32, model.hparams.n_enc_out_chans,
  ...
  std::vector<sam_image_u8> masks
      = sam_postprocess_masks(model.hparams, img.nx, img.ny, st, mask_on_val, mask_off_val);
                               ...
                               int *n_masks, int mask_on_val, int mask_off_val)
{
  ...
  auto masks
      = sam_compute_masks(cpp_img, n_threads, cpp_points, *ctx->state, mask_on_val, mask_off_val);
  if(masks.empty())
  {
    *n_masks = 0;
    return nullptr;
  ...
  *n_masks = masks.size();
  auto *result = new seg_image_t[*n_masks];
