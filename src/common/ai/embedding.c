...
static int _upgrade_embed_schema(sqlite3 *db, int from_version)
{
  ...
  if(sqlite3_prepare_v2(db,
       ...
       -1, &stmt, NULL) != SQLITE_OK)
    return -1;
  sqlite3_bind_int(stmt, 1, EMBED_DB_VERSION);
                                        ...
                                        const dt_ai_model_info_t *info)
{
  ...
  // dynamic-shape model. it clamps the copy to the capacity implied by
  // the shape passed in here, so a wider model is truncated rather than
  // overrunning this buffer, and the width check below then rejects it
...
// into embedding.tag_embeddings with source='model'
static void _import_model_tags(void)
{
  if(!_embed_attached) return;
  ...
  for(int i = 0; i < n; i++)
  {
    ...
    sqlite3_stmt *stmt = NULL;
    DT_DEBUG_SQLITE3_PREPARE_V2(db,
      "INSERT OR IGNORE INTO embedding.tag_embeddings"
