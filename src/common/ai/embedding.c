...
#define EMBEDDING_MODEL_TASK "embedding"
#define EMBEDDING_DB_VERSION 1
#define TAG_MIN_EXAMPLES 3

// embeddings DB is attached to the main database handle as "embedding"
static gboolean _embedding_attached = FALSE;
...
static int _create_auto_tag_table(sqlite3 *db)
{
  return sqlite3_exec(db,
    "CREATE TABLE IF NOT EXISTS embedding.auto_tagged ("
...
{
  return sqlite3_exec(db,
    "CREATE TABLE IF NOT EXISTS embedding.tag_embeddings ("
...
static void _dev_ensure_tag_table_shape(sqlite3 *db)
{
  ...
  if(sqlite3_prepare_v2(db, "PRAGMA embedding.table_info(tag_embeddings)",
  ...
  sqlite3_exec(db, "DROP TABLE IF EXISTS embedding.tag_embeddings",
               NULL, NULL, NULL);
  _create_tag_table(db);
  _create_auto_tag_table(db);
}

static int _upgrade_embedding_schema(sqlite3 *db, int from_version)
{
  if(from_version == EMBEDDING_DB_VERSION)
    return from_version;
  ...
  if(from_version > EMBEDDING_DB_VERSION)
  {
    dt_print(DT_DEBUG_AI,
             ...
             from_version, EMBEDDING_DB_VERSION);
    return -1;
  }
  ...
  if(from_version == 0)
  {
    // same shape as main.db_info and data.db_info in database.c
    int rc = sqlite3_exec(db,
      "CREATE TABLE IF NOT EXISTS embedding.db_info"
    ...
    snprintf(sql, sizeof(sql),
      "CREATE VIRTUAL TABLE IF NOT EXISTS embedding.vec_embeddings "
  ...
  if(sqlite3_prepare_v2(db,
       "INSERT OR REPLACE INTO embedding.db_info (key, value)"
       "  VALUES ('version', ?1)",
       -1, &stmt, NULL) != SQLITE_OK)
    return -1;
  sqlite3_bind_int(stmt, 1, EMBEDDING_DB_VERSION);
  const int rc = sqlite3_step(stmt);
  sqlite3_finalize(stmt);
  if(rc != SQLITE_DONE) return -1;

  return EMBEDDING_DB_VERSION;
}
...
void dt_ai_embedding_init(void)
{
  ...
  // attach embedding.db in same directory as library.db
  char datadir[PATH_MAX] = {0};
  dt_loc_get_user_config_dir(datadir, sizeof(datadir));

  char dbpath[PATH_MAX] = {0};
  snprintf(dbpath, sizeof(dbpath), "%s%sembedding.db",
           datadir, G_DIR_SEPARATOR_S);

  sqlite3_stmt *stmt = NULL;
  rc = sqlite3_prepare_v2(db,
    "ATTACH DATABASE ?1 AS embedding", -1, &stmt, NULL);
  if(rc != SQLITE_OK)
  {
    dt_print(DT_DEBUG_AI,
  ...
  // check schema version and migrate if needed. a fresh file has no
  // db_info table yet, so a failed prepare means version 0
  int version = 0;
  sqlite3_stmt *ver_stmt = NULL;
  if(sqlite3_prepare_v2(db,
       "SELECT value FROM embedding.db_info WHERE key = 'version'",
  ...
  const int new_version = _upgrade_embedding_schema(db, version);
  if(new_version < 0)
  {
    dt_print(DT_DEBUG_AI,
             "[embeddings] schema migration failed");
    sqlite3_exec(db, "DETACH DATABASE embedding",
                 NULL, NULL, NULL);
    return;
  }
                                  ...
                                  gpointer user_data)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    "  AND id NOT IN (SELECT imgid FROM embedding.embeddings)",
    -1, &stmt, NULL);
...
void dt_ai_embedding_cleanup(void)
{
  if(!_embedding_attached) return;
  ...
  sqlite3 *db = dt_database_get(darktable.db);
  if(db)
    sqlite3_exec(db, "DETACH DATABASE embedding", NULL, NULL, NULL);
  _embedding_attached = FALSE;
}

// --- query ---

gboolean dt_ai_embedding_has(dt_imgid_t imgid)
{
  if(!_embedding_attached) return FALSE;

  sqlite3_stmt *stmt = NULL;
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                              "SELECT 1 FROM embedding.embeddings"
                              "  WHERE imgid = ?1",
                              -1, &stmt, NULL);
...
gboolean dt_ai_embedding_has_any(GList *images)
{
  if(!_embedding_attached || !images) return FALSE;
  ...
  char *q = g_strdup_printf(
    "SELECT 1 FROM embedding.embeddings WHERE imgid IN (%s) LIMIT 1",
    ids->str);
...
float *dt_ai_embedding_get(dt_imgid_t imgid, int *dim)
{
  if(!_embedding_attached) return NULL;

  sqlite3_stmt *stmt = NULL;
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                              "SELECT embedding"
                              "  FROM embedding.vec_embeddings"
                              "  WHERE imgid = ?1",
                              -1, &stmt, NULL);
  DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
  ...
  if(sqlite3_step(stmt) == SQLITE_ROW)
  {
    ...
    if(blob && bytes == DT_AI_EMBEDDING_DIM * (int)sizeof(float))
    {
      result = g_malloc(bytes);
      memcpy(result, blob, bytes);
      if(dim) *dim = DT_AI_EMBEDDING_DIM;
    }
  }
  sqlite3_finalize(stmt);
  return result;
...
static gboolean _store_embedding(dt_imgid_t imgid,
                                 ...
                                 const char *version)
{
  if(!_embedding_attached || !embedding) return FALSE;
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(db,
                              "INSERT OR REPLACE INTO embedding.embeddings"
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(db,
                              "INSERT OR REPLACE INTO embedding.vec_embeddings"
                              "  (imgid, embedding)"
                              "  VALUES (?1, ?2)",
                              -1, &stmt, NULL);
  DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, imgid);
  DT_DEBUG_SQLITE3_BIND_BLOB(stmt, 2, embedding,
                             DT_AI_EMBEDDING_DIM * sizeof(float),
                             SQLITE_TRANSIENT);
  rc = sqlite3_step(stmt);
  sqlite3_finalize(stmt);
...
typedef struct _embedding_preproc_t
{
...
// the width the model says it emits. darktable will not guess it: the
// vector index column and every buffer here are sized from
// DT_AI_EMBEDDING_DIM, so a model that disagrees would be truncated into
// an index built for a different embedding space
static gboolean _model_dim_is_usable(const dt_ai_model_info_t *info,
                                     const char *model_id)
{
  const int dim = dt_ai_model_attribute_int(info, "embedding_dim", 0);

  if(dim == 0)
  {
    dt_print(DT_DEBUG_ALWAYS,
             "[embeddings] model %s declares no embedding_dim,"
             " it cannot be used for embeddings",
             model_id);
    return FALSE;
  }

  if(dim != DT_AI_EMBEDDING_DIM)
  {
    dt_print(DT_DEBUG_ALWAYS,
             "[embeddings] model %s emits %d dimensions, this build stores %d,"
             " it cannot be used for embeddings",
             model_id, dim, DT_AI_EMBEDDING_DIM);
    return FALSE;
  }
...
static gboolean _embedding_compute_with_ctx(dt_imgid_t imgid,
                                        dt_ai_context_t *ctx,
                                        const char *model_id,
                                        const dt_ai_model_info_t *info)
{
  if(!_embedding_attached) return FALSE;
  if(dt_ai_embedding_has(imgid)) return TRUE;
  ...
  // dt_ai_run reports the runtime's real dims back through out_shape for
  // a dynamic-shape model, so the width check below sees what the model
  // actually produced rather than what was asked for
  float output[DT_AI_EMBEDDING_DIM];
  int64_t out_shape[] = {1, DT_AI_EMBEDDING_DIM};
  ...
  if(ret == 0 && (out_tensor.ndim != 2
                  || out_shape[1] != DT_AI_EMBEDDING_DIM))
  {
    dt_print(DT_DEBUG_AI,
  ...
  if(!pp.output_normalized)
    _normalize(output, DT_AI_EMBEDDING_DIM);
...
gboolean dt_ai_embedding_compute(dt_imgid_t imgid)
{
  if(!_embedding_attached) return FALSE;

  char *model_id = dt_ai_models_get_active_for_task(EMBEDDING_MODEL_TASK);
  ...
  if(!_model_dim_is_usable(info, model_id))
  {
  ...
  const gboolean ok = _embedding_compute_with_ctx(imgid, ctx, model_id, info);
...
typedef struct _embedding_job_t
{
  GList *images;
} _embedding_job_t;

static int32_t _embedding_job_run(dt_job_t *job)
{
  _embedding_job_t *j = dt_control_job_get_params(job);
  if(!j) return 1;
  ...
  for(GList *l = j->images; l; l = g_list_next(l))
  {
    ...
    if(_embedding_compute_with_ctx(imgid, ctx, model_id, info) && auto_tag)
  ...
  if(auto_tag && indexed && !cancelled)
  {
    ...
    dt_ai_embedding_update_user_tags();
    dt_ai_embedding_calibrate_tags();
    _evaluate_user_tags();
    for(GList *l = indexed; l; l = g_list_next(l))
      dt_ai_embedding_auto_tag(GPOINTER_TO_INT(l->data));

    // dt_tag_attach() with undo_on = FALSE raises nothing, so without
    // this the keywords panel and collect module keep showing the
    // library as it was before the batch
...
static void _embedding_job_cleanup(void *param)
{
  _embedding_job_t *j = param;
...
// hard cap per image. a flat vocabulary puts every tag in a group of its
// own, so the per-group cap never binds there, and the z-score floor on
// its own does not bound how many tags clear it
#define TAG_MAX_PER_IMAGE 5
...
static void _store_user_tag(sqlite3 *db, int tagid,
                            ...
                            int count)
{
  float embedding[DT_AI_EMBEDDING_DIM];
  memcpy(embedding, sum, sizeof(embedding));
  _normalize(embedding, DT_AI_EMBEDDING_DIM);

  sqlite3_stmt *stmt = NULL;
  DT_DEBUG_SQLITE3_PREPARE_V2(db,
    "INSERT OR REPLACE INTO embedding.tag_embeddings"
...
void dt_ai_embedding_update_user_tags(void)
{
  if(!_embedding_attached) return;
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(db,
    ...
    "  JOIN embedding.embeddings e ON e.imgid = ti.imgid"
    " WHERE t.name NOT LIKE 'darktable|%'"
    "   AND e.model_id = ?2"
    "   AND NOT EXISTS (SELECT 1 FROM embedding.auto_tagged a"
  ...
  while(sqlite3_step(stmt) == SQLITE_ROW)
  {
    ...
    float sum[DT_AI_EMBEDDING_DIM];
    memset(sum, 0, sizeof(sum));

    sqlite3_stmt *vec_stmt = NULL;
    DT_DEBUG_SQLITE3_PREPARE_V2(db,
      "SELECT v.embedding FROM embedding.vec_embeddings v"
      "  JOIN main.tagged_images ti ON ti.imgid = v.imgid"
      "  JOIN embedding.embeddings e ON e.imgid = v.imgid"
      " WHERE ti.tagid = ?1"
      "   AND e.model_id = ?2"
      "   AND NOT EXISTS (SELECT 1 FROM embedding.auto_tagged a"
    ...
    while(sqlite3_step(vec_stmt) == SQLITE_ROW)
    {
      ...
      if(!vec
         || bytes != DT_AI_EMBEDDING_DIM * (int)sizeof(float))
        continue;
      for(int d = 0; d < DT_AI_EMBEDDING_DIM; d++)
        sum[d] += vec[d];
      n++;
    }
  ...
  while(sqlite3_step(tags) == SQLITE_ROW)
  {
    ...
    if(!sum || n < 2
       || sqlite3_column_bytes(tags, 1)
            != DT_AI_EMBEDDING_DIM * (int)sizeof(float))
      continue;
    ...
    float full[DT_AI_EMBEDDING_DIM];
    memcpy(full, sum, sizeof(full));
    _normalize(full, DT_AI_EMBEDDING_DIM);
    ...
    while(sqlite3_step(ex) == SQLITE_ROW)
    {
      ...
      _normalize(loo, DT_AI_EMBEDDING_DIM);
      double dot_loo = 0.0;
      for(int d = 0; d < DT_AI_EMBEDDING_DIM; d++)
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(db,
      "UPDATE embedding.tag_embeddings"
...
void dt_ai_embedding_calibrate_tags(void)
{
  if(!_embedding_attached) return;
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(db,
    ...
    "SELECT embedding FROM embedding.vec_embeddings"
  ...
  while(sqlite3_step(stmt) == SQLITE_ROW)
  {
    ...
    if(v && sqlite3_column_bytes(stmt, 0)
             == DT_AI_EMBEDDING_DIM * (int)sizeof(float))
    {
      float *copy = g_malloc(DT_AI_EMBEDDING_DIM * sizeof(float));
      memcpy(copy, v, DT_AI_EMBEDDING_DIM * sizeof(float));
  ...
  // only user tags are ever scored against an image, so calibrating any
  // other source is a full pass over the sample for nothing
  DT_DEBUG_SQLITE3_PREPARE_V2(db,
    "SELECT tagid, embedding FROM embedding.tag_embeddings"
    " WHERE source = 'user'",
    -1, &stmt, NULL);
  while(sqlite3_step(stmt) == SQLITE_ROW)
  {
...
void dt_ai_embedding_auto_tag(dt_imgid_t imgid)
{
  if(!_embedding_attached) return;

  int dim = 0;
  float *embedding = dt_ai_embedding_get(imgid, &dim);
  if(!embedding || dim != DT_AI_EMBEDDING_DIM)
  {
    g_free(embedding);
    return;
  }
  ...
  char *active = dt_ai_models_get_active_for_task(EMBEDDING_MODEL_TASK);
  sqlite3_stmt *chk = NULL;
  gboolean same_model = FALSE;
  DT_DEBUG_SQLITE3_PREPARE_V2(db,
    "SELECT model_id FROM embedding.embeddings WHERE imgid = ?1",
    -1, &chk, NULL);
  ...
  // take each group's best, then the best of those: the per-group cap
  // stops one branch of a hierarchy filling every slot, and the total
  // cap covers the flat case where every tag is its own group
  GArray *winners = g_array_new(FALSE, FALSE, sizeof(_tag_hit_t));
  GHashTableIter iter;
  gpointer key, value;
  ...
  while(g_hash_table_iter_next(&iter, &key, &value))
  {
    ...
    for(guint i = 0; i < k; i++)
    {
      const _tag_hit_t hit = g_array_index(hits, _tag_hit_t, i);
      g_array_append_val(winners, hit);
    }
  }
  g_hash_table_destroy(groups);

  g_array_sort(winners, _hit_cmp);
  const guint keep = MIN((guint)TAG_MAX_PER_IMAGE, winners->len);

  int applied = 0;
  for(guint i = 0; i < keep; i++)
  {
    const int tagid = g_array_index(winners, _tag_hit_t, i).tagid;
    dt_tag_attach(tagid, imgid, FALSE, FALSE);

    // remember it was us, not the user
    sqlite3_stmt *rec = NULL;
    DT_DEBUG_SQLITE3_PREPARE_V2(db,
      "INSERT OR IGNORE INTO embedding.auto_tagged (imgid, tagid)"
...
void dt_ai_embedding_batch(GList *images)
{
  if(!images || !_embedding_attached) return;

  _embedding_job_t *j = g_new0(_embedding_job_t, 1);
  j->images = g_list_copy(images);

  dt_job_t *job = dt_control_job_create(_embedding_job_run,
                                        "ai embed");
  dt_control_job_set_params(job, j, _embedding_job_cleanup);
  dt_control_job_add_progress(job, _("indexing images"), TRUE);
  dt_control_add_job(DT_JOB_QUEUE_USER_BG, job);
}

void dt_ai_embedding_remove(GList *images)
{
  if(!images || !_embedding_attached) return;
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(db,
                              "DELETE FROM embedding.embeddings WHERE imgid = ?1",
                              -1, &stmt_meta, NULL);
  DT_DEBUG_SQLITE3_PREPARE_V2(db,
                              "DELETE FROM embedding.vec_embeddings WHERE imgid = ?1",
                              -1, &stmt_vec, NULL);
