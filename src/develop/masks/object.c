...
#define MIN_OBJECT_RADIUS 0.0005f
#define MIN_OBJECT_BORDER 0.0005f
                                         ...
                                         const int index)
{
  ...
  // add a preview when creating a object
  if(gui->creation)
  {
    if(dt_modifier_is(state, GDK_SHIFT_MASK))
    {
      const float masks_border = dt_masks_change_size
        (up,
         dt_conf_get_float(DT_MASKS_CONF(form->type, object, border)),
         DT_MASKS_OBJECT, max_mask_border);

      dt_conf_set_float(DT_MASKS_CONF(form->type, object, border), masks_border);
    ...
    else if(dt_modifier_is(state, 0))
    {
      const float masks_size = dt_masks_change_size
        (up,
         dt_conf_get_float(DT_MASKS_CONF(form->type, object, size)),
         MIN_OBJECT_RADIUS,
         max_mask_size);

      dt_conf_set_float(DT_MASKS_CONF(form->type, object, size), masks_size);
  ...
  if(gui->form_selected)
  {
    ...
    {
      dt_masks_point_object_t *object = form->points->data;
      // resize don't care where the mouse is inside a shape
      if(dt_modifier_is(state, GDK_SHIFT_MASK))
      {
        object->border = dt_masks_change_size
          (up,
           object->border,
           DT_MASKS_OBJECT, max_mask_border);
        ...
        dt_conf_set_float(DT_MASKS_CONF(form->type, object, border), object->border);
        dt_toast_log(_("feather size: %3.2f%%"), object->border*100.0f);
      }
      else if(gui->edit_mode == DT_MASKS_EDIT_FULL)
      {
        object->radius = dt_masks_change_size
          (up,
           object->radius,
           DT_MASKS_OBJECT, max_mask_border);
        ...
        dt_dev_add_masks_history_item(darktable.develop, module, TRUE);
        dt_masks_gui_form_create(form, gui, index, module);
        dt_conf_set_float(DT_MASKS_CONF(form->type, object, size), object->radius);
        dt_toast_log(_("size: %3.2f%%"), object->radius*100.0f);
      }
      else
      {
        return 0;
                                         ...
                                         const int index)
{
  ...
  else
  {
    ...
    dt_masks_point_object_t *object = malloc(sizeof(dt_masks_point_object_t));

    // we change the center value
    float pts[2] = { pzx * wd, pzy * ht };
    dt_dev_distort_backtransform(darktable.develop, pts, 1);
    object->anchor[0] = pts[0] / iwidth;
    object->anchor[1] = pts[1] / iheight;
    object->label = 1;
    ...
    // }
    // object->radius = dt_conf_get_float(DT_MASKS_CONF(form->type, object, size));
    // object->border = dt_conf_get_float(DT_MASKS_CONF(form->type, object, border));
    form->points = g_list_append(form->points, object);
    ...
    // // if we draw a clone object, we start now the source dragging
                                          ...
                                          const int index)
{
  ...
  //   dt_dev_distort_backtransform(darktable.develop, pts, 1);
  //   object->center[0] = pts[0] / iwidth;
  //   object->center[1] = pts[1] / iheight;
  //   dt_dev_add_masks_history_item(darktable.develop, module, TRUE);
                                      ...
                                      const int index)
{
  ...
  if(gui->point_dragging >= 1)
  {
    ...
    object->radius = CLAMP(object->radius * s, MIN_OBJECT_RADIUS, max_mask_size);
  ...
  //   object->border = CLAMP((object->radius + object->border) * s - object->radius,
  ...
  else if(!gui->creation)
  {
    ...
    int near;
    float dist;
    _object_get_distance(x, y, as, gui, index, 0, &in, &inb, &near, &ins, &dist);
    // if(ins)
    // {
    //   gui->form_selected = TRUE;
    //   gui->source_selected = TRUE;
...
// static void _object_draw_lines(const gboolean borders,
                                   ...
                                   int *points_count)
{
  ...
  *points_count = l + 1;
  ...
  const float anchor_x = x * wd;
  const float anchor_y = y * ht;
  points[0] = anchor_x;
  points[1] = anchor_y;
  ...
  for(int i = 1; i < l + 1; i++)
  {
    ...
    points[i * 2] = anchor_x + r * cosf(alpha);
    points[i * 2 + 1] = anchor_y + r * sinf(alpha);
  }
  return points;
}

static int _object_get_points_source(dt_develop_t *dev,
                                     const float x,
                                     const float y,
                                     const float xs,
                                     const float ys,
                                     ...
                                     const dt_iop_module_t *module)
{
  ...
  // compute the points of the target (center and circumference of object)
...
static int _object_get_points(dt_develop_t *dev,
                              ...
                              int *points_count)
{
  (void)radius; // keep compiler from complaining about unused arg
  (void)radius2;
  (void)rotation;
  float wd, ht;
  dt_masks_get_image_size(NULL, NULL, &wd, &ht);

  // compute the points we need to transform (center and circumference of object)
  *points = _points_to_transform(x, y, radius, wd, ht, points_count);
  if(!*points) return 0;
                                       ...
                                       const int num_points)
{
  ...
  if(gpt->points_count > 0)
  {
    for(int i = 0; i < gpt->points_count; i++)
    {
      dt_masks_draw_anchor(cr, gui->point_dragging > 0 || gui->point_selected > 0,
  ...
  if(gui->creation)
  {
    if(gui->guipoints_count == 0)
    {
      ...
      // float radius_a = dt_conf_get_float(DT_MASKS_CONF(form->type, object, size));
      // float radius_b = dt_conf_get_float(DT_MASKS_CONF(form->type, object, border));
      // radius_b += radius_a;
      ...
      // float *border = NULL;
      // int border_count = 0;
      _object_get_points(darktable.develop, x, y,
                         0.0, 0.0, 0.0, &points, &points_count);
      // if(draw && radius_a != radius_b)
      // {
      //   draw = _object_get_points(darktable.develop, x, y,
      ...
      _object_draw_lines(FALSE, FALSE, cr,
                         FALSE, zoom_scale, points, points_count);
      // we draw the borders
      // _object_draw_lines(TRUE, FALSE, cr,
      ...
      //   x = 0.0f;
      //   y = 0.0f;
      //   dt_masks_calculate_source_pos_value(gui, DT_MASKS_OBJECT,
      //                                       gui->posx, gui->posy,
      //                                       gui->posx, gui->posy,
      //                                       &x, &y, FALSE);
  ...
  // {
  //   // compute the dest inner object intersection with the line from
  ...
  //   _object_draw_lines(FALSE, TRUE, cr, selected,
                                     ...
                                     const dt_iop_module_t *module)
{
  ...
  const float x = object->center[0];
  const float y = object->center[1];

  if(source)
  {
    const float xs = form->source[0];
    const float ys = form->source[1];
    return _object_get_points_source(dev, x, y, xs, ys,
                                     object->radius, object->radius, 0,
                                     points, points_count,
                                     module);
  }
  else
  {
                                   ...
                                   points, points_count))
    {
      if(border)
      {
        const float outer_radius = object->radius + object->border;
                            ...
                            int *posy)
{
  ...
  int num_points;
  float *const restrict points =
    _points_to_transform(object->center[0], object->center[1],
                         outer_radius, wd, ht, &num_points);
  if(points == NULL)
    return 0;
                            ...
                            int *posy)
{
  ...
  // we get the area
  if(!_object_get_area(module, piece, form, width, height, posx, posy)) return 0;

  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object area took %0.04f sec",
           form->name, dt_get_lap_time(&start2));

  // we get the object values
  dt_masks_point_object_t *const restrict object = form->points->data;
  ...
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object draw took %0.04f sec", form->name, dt_get_lap_time(&start2));
  ...
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object transform took %0.04f sec", form->name,
  ...
  *buffer = dt_alloc_align_float((size_t)w * h);
  ...
  const float centerx = object->center[0] * wi;
  const float centery = object->center[1] * hi;
  const float radius2 = object->radius * mindim * object->radius * mindim;
  const float total2 = (object->radius + object->border) * mindim
    * (object->radius + object->border) * mindim;
  ...
  for(int i = 0 ; i < h*w; i++)
  {
    ...
    // quadratic falloff between the object's radius and the radius of
    // the outside of the feathering
    const float ratio = (total2 - l2) / border2;
    // enforce 1.0 inside the object and 0.0 outside the feathering
  ...
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object fill took %0.04f sec",
           form->name, dt_get_lap_time(&start2));
                                ...
                                float *const restrict buffer)
{
  ...
  const int mindim = MIN(wi, hi);
  const float radius2 = object->radius * mindim * object->radius * mindim;
  const float total = (object->radius + object->border) * mindim;
  ...
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object init took %0.04f sec",
           form->name, dt_get_lap_time(&start2));

  // we look at the outer object of the shape - no effects outside of
  // this object; we need many points as we do not know how the object
  ...
  // we transform the outer object from input image coordinates to current point in pixelpipe
  if(!dt_dev_distort_transform_plus(module->dev, piece->pipe, module->iop_order,
  ...
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object outline took %0.04f sec",
           form->name, dt_get_lap_time(&start2));
  ...
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object bounding box took %0.04f sec",
           form->name, dt_get_lap_time(&start2));
  ...
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object grid took %0.04f sec", form->name, dt_get_lap_time(&start2));
  ...
  for(int j = 0; j < bbh; j++)
    for(int i = 0; i < bbw; i++)
    {
      ...
      // quadratic falloff between the object's radius and the radius
  ...
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object draw took %0.04f sec", form->name,
           dt_get_lap_time(&start2));
  ...
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           ...
           form->name, dt_get_lap_time(&start2));
  dt_print(DT_DEBUG_MASKS | DT_DEBUG_PERF,
           "[masks %s] object total render took %0.04f sec", form->name,
...
{
  dt_conf_get_and_sanitize_float(DT_MASKS_CONF(type, object, size), MIN_OBJECT_RADIUS, 0.5f);
  dt_conf_get_and_sanitize_float(DT_MASKS_CONF(type, object, border), DT_MASKS_OBJECT, 0.5f);
}
                                    ...
                                    float *max)
{
  ...
  dt_masks_point_object_t *object = form->points ? form->points->data : NULL;

  float masks_size = object
    ? object->radius
    : dt_conf_get_float(DT_MASKS_CONF(form->type, object, size));
  ...
  switch(prop)
  {
    case DT_MASKS_PROPERTY_SIZE:;
      const float max_mask_size =
        form->type & (DT_MASKS_CLONE | DT_MASKS_NON_CLONE) ? 0.5f : 1.0f;
      masks_size = CLAMP(masks_size * ratio, MIN_OBJECT_RADIUS, max_mask_size);

      if(object) object->radius = masks_size;
      dt_conf_set_float(DT_MASKS_CONF(form->type, object, size), masks_size);

      *sum += masks_size;
      *max = fminf(*max, max_mask_size / masks_size);
      *min = fmaxf(*min, MIN_OBJECT_RADIUS / masks_size);
      ++*count;
      break;
    case DT_MASKS_PROPERTY_FEATHER:;
      ...
      masks_border = CLAMP(masks_border * ratio, DT_MASKS_OBJECT, max_mask_border);

      if(object) object->border = masks_border;
      dt_conf_set_float(DT_MASKS_CONF(form->type, object, border), masks_border);

      *sum += masks_border;
      *max = fminf(*max, max_mask_border / masks_border);
      *min = fmaxf(*min, DT_MASKS_OBJECT / masks_border);
      ++*count;
      break;
    default:;
  }
...
static void _object_initial_source_pos(const float iwd,
                                       ...
                                       float *y)
{
  const float radius = MIN(0.5f, dt_conf_get_float("plugins/darkroom/spots/object_size"));
